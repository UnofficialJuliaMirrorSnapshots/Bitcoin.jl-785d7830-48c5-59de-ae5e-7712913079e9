...
@testset "Script" begin
    ...
    @testset "Evaluate" begin
        ...
        # TODO Switch to BitcoinPrimitives.Tx
        tx = BitcoinPrimitives.Tx(stream)
        tx_in = tx.inputs[1]
        combined_script = Bitcoin.Script(nothing)
        append!(combined_script.instructions, copy(tx_in.scriptsig.data))
        append!(combined_script.instructions, copy(Bitcoin.script_pubkey(tx_in).data))
